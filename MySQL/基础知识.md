...
> InnoDB 实现的 REPEATABLE-READ 隔离级别其实是可以解决大部分幻读问题发生的，主要有下面两种情况：
>
...
- `using MRR`: MRR 是 MySQL 用于 **减少随机磁盘 I/O** 的一种优化技术，主要用于在读取范围内的多行数据时，提升性能：
  1. **批量读取**：将随机的、离散的记录读取操作转为顺序的、批量的读取操作，减少磁盘的随机访问。
  2. **排序读取**：将通过非聚簇索引（secondary index）读取的数据，按照主键排序后，再从聚簇索引（primary index）中读取对应的数据。
  3. MRR 主要在以下场景中被使用：
     1. **范围扫描（Range Scan）**：当查询条件使用范围查询（如 `BETWEEN`、`>`, `<` 等）时，会触发 MRR 优化。
     2. **非聚簇索引访问**：当查询需要通过非聚簇索引（secondary index）查找数据，并从聚簇索引（primary index）中获取完整记录时，使用 MRR 能够提升性能。
