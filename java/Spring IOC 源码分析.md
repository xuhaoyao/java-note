...
**为什么要使用 ioc** ： 目的就是减少对代码的改动，实现解耦合。 在实际项目中一个 Service 类可能依赖了很多其他的类，假如我们需要**实例化这个 Service，可能要每次都要搞清这个 Service 所有底层类的构造函数，每个依赖的类要怎样构造，怎样传参**，这可能会把人逼疯。如果利用 IoC 的话，只需要配置好每个Bean，然后在需要的地方引用就行了，这大大增加了项目的可维护性且降低了开发难度。
